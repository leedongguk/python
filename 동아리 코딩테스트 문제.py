...
class Game:
    ...
    def start_game(self):
      ...
      # Write code here..
      a = input("당신의 캐릭터 번호를 선택해주세요 (1,2,3,4) :")
      a = int(a)
      print("당신의 캐릭터는 " + self.player[a-1].name + "입니다.")
      self.user_character = self.player[a-1].name
      ...
      for i in range(10):
         ...
         while a in self.answer_str:
            a = str(random.choice(string.ascii_uppercase))
         self.answer_str.append(a)
      self.answer_str.sort()
      print("랜덤으로 생성된 답입니다. 플레이어는 알 수 없습니다.")
      for i in range(10):
        print(self.answer_str[i], end='')
      print(" ")
    ...
    def play_game(self):
      ...
      for i in range(4):
        ...
        # 이 단계에서 플레이어 별 점수 집계도 처리해주셔야 합니다.
          if a in self.answer_str:
           for s in range(10):
            if a == self.answer_str[s]:
                 self.current_str[s] = a
                 print("***** 맞았습니다 ᵔεᵔ  *****")
          ...
          # TODO 3-(3) : 오답 시, 생명력을 데미지만큼 감소시켜주고 이를 출력해주세요.
             ...
             print(self.player[i].name + "님은 틀렸기 때문에 HP가 "+ str(self.player[i].hp) + "남았습니다.")
          # Write code here..
    ...
    def game_result(self):
      ...
      self.player.sort(key=lambda x : x.hp, reverse=True)
      for i in range(4):
        if self.player[i].name == self.user_character and self.player[i].hp <= 0:
            print(str(i+1)+"등 : *" + str(self.player[i].name) + "*    (사망)")
        elif self.player[i].name == self.user_character:
            print(str(i+1)+"등 : *" + str(self.player[i].name) + "*    (HP: " + str(self.player[i].hp) + ")")
        elif self.player[i].name != self.user_character and self.player[i].hp <= 0:
            print(str(i+1)+"등 : " + str(self.player[i].name) + "    (사망)")
        else:
            print(str(i+1)+"등 : " + str(self.player[i].name) + "    (HP: " + str(self.player[i].hp) + ")")
      ...
      for i in range(4):
        if self.player[i].name == self.user_character:
            print(str(i+1)+"등 : *" + str(self.player[i].name) + "*   " + str(self.player[i].correct_alp) + "회")
        else:
            print(str(i+1)+"등 : " + str(self.player[i].name) + "    " + str(self.player[i].correct_alp) + "회")
